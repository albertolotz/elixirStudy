defmodule ExMon.PokeApi.ClientTest do
  ...
  describe "get_pokemon/1" do
    test "when  there is a pokemon with give name, returns the pokemon" do
    ...
    test "when a pokemon isn't found with given name, returns the error" do
      mock(fn %{method: :get, url: @base_url <> "pokemon_that_not_exist"} ->
        %Tesla.Env{status: 404}
      end)

      response = Client.get_pokemon("pokemon_that_not_exist")
    ...
    test "when there is a unexpected error, return the error" do
      mock(fn %{method: :get, url: @base_url <> "pikachu"} ->
        {:error, :timeout}
      end)
